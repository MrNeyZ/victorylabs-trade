 ...
 * positions, recent trades, recent history events. A syntactically valid
 * pubkey we simply have no data for is not an error — it returns 200 with
 * `profile: null` and empty arrays, not a 404 (this is a query over an
 * open identifier space, not a lookup of a resource that must exist).
 */
...
walletsRouter.get('/:walletPubkey', async (req, res) => {
  ...
  const [profile, positions, recentTrades, recentHistory] = await Promise.all([
